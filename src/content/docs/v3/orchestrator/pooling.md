...
A remote sets an optional `pool` on a shared package in its `federation.config.mjs`. It mirrors `shareScope` in shape, and Core (since v4.3) passes it through to `remoteEntry.json` untouched — the build itself does nothing with it. See [Core — per-package options](/docs/v4/core/sharing/#per-package-options).
...
- [Core — sharing dependencies](/docs/v4/core/sharing/) — the build-side config that emits `shared` entries, including the `pool` field.
