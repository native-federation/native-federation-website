...
Both shell and remotes need `@angular/localize`. Note that `@angular/localize` is in the core's [DEFAULT_SKIP_LIST](configuration.md#default_skip_list) — it's deliberately _not_ shared, because it patches globals at boot.
...
- [Builder → Output paths and locales](builder.md#output-paths-and-locales) — how the federation builder lays out locale folders.
