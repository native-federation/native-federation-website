...
Both shell and remotes need `@angular/localize`. Note that `@angular/localize` is in the adapter's [NG_SKIP_LIST](/docs/v4/angular-adapter/configuration/#ng_skip_list) — it's deliberately _not_ shared, because it patches globals at boot.
...
- [Builder → Locale-aware output paths](/docs/v4/angular-adapter/builder/#locale-aware-output-paths) — how the federation builder lays out locale folders.
