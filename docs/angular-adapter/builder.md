...
| `integrity` | `boolean` | `false` | Forwards to the core's [`integrity` option](../core/build-process.md) — emit SRI hashes for every shared external, exposed module and chunk under a top-level `integrity` map in `remoteEntry.json`. The orchestrator copies these onto the import map so the browser can verify each module before executing it. See [Subresource Integrity](../orchestrator/security.md#subresource-integrity). |
